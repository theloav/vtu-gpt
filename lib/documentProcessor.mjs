...
// Explicitly configure global PDFJS properties to control pdfjs-dist behavior
// This is a more direct way to influence pdfjs-dist when pdf-parse options aren't enough
if (typeof global.PDFJS === 'undefined') {
  global.PDFJS = {};
}
// Point to the correct worker script if needed, although canvasFactory should prevent worker usage for rendering
// It's crucial that this path is resolvable by the serverless environment
global.PDFJS.workerSrc = require.resolve('pdfjs-dist/build/pdf.worker.js'); // Use require.resolve for CommonJS style path resolution
