...
import pdfParse from 'pdf-parse'; // Reverting to static import, will try `pdfParse` directly

// Aggressive polyfills for missing DOM APIs in Node.js/Vercel environment
// This is to combat the persistent "DOMMatrix is not defined" error.
if (typeof globalThis.DOMMatrix === 'undefined') {
  globalThis.DOMMatrix = class DOMMatrix {
    constructor() { /* minimal implementation */ }
    scale() { return this; }
    ...
    switch (fileExtension) {
      case '.pdf':
        ...
        const pdfData = await pdfParse(fileBuffer, {
          // Pass a dummy canvas factory to pdfjs-dist options
          // This prevents pdfjs-dist from attempting to load native canvas modules directly
          // Instead, it will use a minimalist, non-rendering canvas for internal operations.
          // This is a common workaround for environments where native canvas is problematic.
          // This function essentially mocks the canvas required by pdf.js
          // Reference: https://github.com/mozilla/pdf.js/blob/master/examples/node/pdf2png/pdf2png.js
          canvasFactory: {
            create: (width, height) => {
              return {
                getContext: (type) => ({
                  ...
                  strokeStyle: '#000000',
                  fillStyle: '#000000',
                  // DOMMatrix, ImageData, Path2D polyfills might still be needed in some contexts
                  // but this factory attempts to provide a minimal interface.
                }),
                width,
                height,
                style: {},
          ...
          // Additional options to minimize rendering dependency
          maxImageSize: 0, // Prevent image rendering
          disableWorker: false, // Keep worker enabled
          disableFontFace: false, // Keep font rendering if possible
          disableNativeImageDecoder: true, // Explicitly disable native image decoder
        });
...
function chunkFacultyCabinData(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (ttsMatch) {
      ...
      if (entryLines.length > 0) {
        ...
        const nameMatch = line.match(/Faculty\s*Name\s*:\s*(.+)/i);
        if (nameMatch) {
          currentEntry.name = nameMatch[1].trim();
        }
        const designationMatch = line.match(/Designation\s*:\s*(.+)/i);
        if (designationMatch) {
          currentEntry.designation = designationMatch[1].trim();
          if (currentEntry.designation.toLowerCase().includes('hod')) {
            currentEntry.isHOD = true;
          }
        }
        const departmentMatch = line.match(/Department\s*:\s*(.+)/i);
        if (departmentMatch) {
          currentEntry.department = departmentMatch[1].trim();
        }
        const cabinMatch = line.match(/Cabin\s*ID\s*:?\s*([A-Z]\d+\/\d+)/i);
        if (cabinMatch) {
          currentEntry.cabin = cabinMatch[1];
        }
      }
    }

    // If entry gets too large (>1500 chars), save it as a general faculty chunk
    if (entryLines.join('\n').length > 1500 && Object.keys(currentEntry).length > 0) {
      ...
      chunks.push({
        ...
        isHOD: currentEntry.isHOD || false,
        cabin: currentEntry.cabin,
        dataType: 'faculty_info'
      });
      currentEntry = {};
      entryLines = [];
    }
  ...
  // Add last entry as a general faculty chunk
  if (Object.keys(currentEntry).length > 0 && entryLines.length > 0) {
    const entryText = entryLines.join('\n');
    chunks.push({
      id: chunkIndex++,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const cabinMatch = line.match(/Cabin\s*ID\s*:?\s*([A-Z]\d+\/\d+)/i);
    if (cabinMatch) {
      if (currentRoomChunk && currentRoomId) {
        chunks.push({
          id: chunkIndex++,
          text: `Room ${currentRoomId} Information:\n${currentRoomChunk}`.trim(),
          length: currentRoomChunk.length,
          room: currentRoomId,
          dataType: 'room_info'
        });
      }
      currentRoomId = cabinMatch[1];
      currentRoomChunk = line + '\n';
    } else {
      if (currentRoomId) {
        currentRoomChunk += line + '\n';
      }
    ...
    if (currentRoomChunk.length > 1500 && currentRoomId) {
      chunks.push({
        id: chunkIndex++,
        text: `Room ${currentRoomId} Information:\n${currentRoomChunk}`.trim(),
        length: currentRoomChunk.length,
  ...
  console.log(`✅ Created ${chunks.length} chunks for faculty cabin data (${chunks.filter(c => c.dataType === 'faculty_info').length} faculty, ${chunks.filter(c => c.dataType === 'room_info').length} rooms)`);
  return chunks;
}
