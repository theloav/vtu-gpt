...
export async function extractTextFromFile(filePath, originalFilename) {
  try {
    ...
    switch (fileExtension) {
      case '.pdf':
        console.log('📄 Processing PDF file with pdf-parse using dynamic import...');
        // Dynamically import pdf-parse to correctly handle its module export in Vercel environment
        const pdfParseModule = await import('pdf-parse');
        const pdfParseFunction = pdfParseModule.default || pdfParseModule; // Use .default if available, otherwise the module itself

        const pdfData = await pdfParseFunction(fileBuffer, {
